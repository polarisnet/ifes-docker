...
enableOverflow: true,
				...
				items:[{
					...
					text: 'Delete Alert',
					handler: function(){
						Ext.MessageBox.confirm('Confirm', 'Are you sure you want to delete selected alert?', function(btn){
							if(btn == 'yes'){
								actionRequest.request({
									...
									success: function(response){
										...
										Ext.Msg.alert('Alert List', '<p class="fix-x-multiline-msg">'+feedback.message+'</p>');
										listStore.reload();
									},
									failure: function(response){
										Ext.Msg.alert('Alert List', 'Oops.. something wrong with the connection to server. Please try again.');
									}
								});
							}
						});
					...
					pressed: false,
					handler: function(){
						gridDeleteState('alerts_listing', JS_USERID, this, listStore);
					},
					scope: this
				}]
			},{
			...
			listeners:{
				...
				statesave: function (objInit, state, eOpts) {
					if(!objInit.isStateRestoring) {
						gridSaveState('alerts_listing', JS_USERID, this, listStore);
					}
				...
				render: function (objInit) {
					...
					gridRestoreState('alerts_listing', JS_USERID, this, listStore);
				},
