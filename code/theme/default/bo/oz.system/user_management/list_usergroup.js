Ext.namespace('Usergroup', 'ListPanel');
...
enableOverflow: true,
				...
				items:[{
					...
					handler: function(){
						Ext.MessageBox.confirm('Confirm', 'Are you sure you want to delete selected user groups?', function(btn){
							if(btn == 'yes'){
								actionRequest.request({
									...
									success: function(response){
										...
										Ext.Msg.alert('User Groups List', '<p class="fix-x-multiline-msg">'+feedback.message+'</p>');
										listStore.reload();
									},
									failure: function(response){
										Ext.Msg.alert('User Groups List', 'Oops.. something wrong with the connection to server. Please try again.');
									}
								});
							}
						});
					...
					pressed: false,
					handler: function(){
						gridDeleteState('usergroup_listing', JS_USERID, this, listStore);
					},
					scope: this
				}]
			},{
			...
			listeners:{
				...
				statesave: function (objInit, state, eOpts) {
					if(!objInit.isStateRestoring) {
						gridSaveState('usergroup_listing', JS_USERID, this, listStore);
					}
				...
				render: function (objInit) {
					...
					gridRestoreState('usergroup_listing', JS_USERID, this, listStore);
				},
