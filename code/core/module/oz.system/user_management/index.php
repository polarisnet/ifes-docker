<?php
	...
	switch(MODULE_UID){
		case 'oz.system.user_management.users.new':
			if(!empty($_GET)){
		...
		break;
		case 'oz.system.user_management.users.view':
		case 'oz.system.user_management.users.edit':
			if(!empty($_GET)){
				$redirect = checkParam('redir');
				if($redirect == 'new'){
					$decryptKey = getCookieValue('added_key');
			...
			if($decryptKey == ''){
				header("Location: ".getModuleURL('oz.system.user_management.users.list')."?invalid=1");
				exit;
			}else{
			...
			if(empty($userData)){
				header("Location: ".getModuleURL('oz.system.user_management.users.list')."?invalid=2");
				exit;
			}
			
			$allowView 		= checkAccess('oz.system.user_management.users.view');
			$allowDelete 	= checkAccess('oz.system.user_management.users.delete');
			$allowEdit 		= checkAccess('oz.system.user_management.users.edit');
			
			if(MODULE_UID == 'oz.system.user_management.users.view'){
				$mode = "view";
			}else{
				$mode = "edit";
			}
			...
			$setting['load_tile'] = "0";
			$setting['left_uid'] = MODULE_PARENT_UID;
			$setting['center_dir'] = DIR_ACTIVE_THEME."/oz.system/user_management/view_user.php";
			if($allowEdit){
		...
		break;
		case 'oz.system.user_management.users.resetpassword': 
			if(!empty($_GET)){
				$redirect = checkParam('redir');
				if($redirect == 'resetpassword'){
					$decryptKey = getCookieValue('added_key');
					if($decryptKey != ""){
						$decryptKey = encryption(getCookieValue('added_key'), $_SESSION['salt'], false);
						$data = getUserSpecificField($decryptKey, "`username`");
						if(!empty($data)){
							$message['content'] = ucfirst($data['username'])." password has been reset successfully.";
							deleteCookieValue('added_key');
						}
					}
				}
			...
			$setting['load_tile'] = "0";
			$setting['left_uid'] = MODULE_PARENT_UID;
			$setting['center_dir'] = DIR_ACTIVE_THEME."/oz.system/user_management/reset_password.php";
			...
			if(!empty($_POST)){
				...
				$newData['password'] 		= hashPassword($newData['username'], $formPassword, $userData['salt']);				
				$newData['modified_by'] 	= $_SESSION['user_id'];
				$newData['modified_date'] 	= date("Y-m-d H:i:s");
					
				if($objUser->updateUser($newData)){
					insertAuditTrails('oz.system.user_management.users.resetpassword', 'resetpassword', "", $userData, $newData);					
					$encInsertedId = encryption($userData['id'], $_SESSION['salt'], true);
					setCookieValue($encInsertedId, 'added_key');					
					header("Location: ".getModuleURL('oz.system.user_management.users.list'));
					exit;
				}
			}
		break;
		case 'oz.system.user_management.users.changepassword': 
			if(!empty($_GET)){
				$redirect = checkParam('redir');
			...
			$setting['load_tile'] = "0";
			$setting['left_uid'] = MODULE_PARENT_UID;
			$setting['center_dir'] = DIR_ACTIVE_THEME."/oz.system/user_management/change_password.php";
		...
		break;
		case 'oz.system.user_management.users.list':
			$HTTP_AJAX = HTTP_ACTIVE_PARENT.'/ajax';
			insertTracker(MODULE_NAME);
			$setting['load_tile'] = "0";
			$setting['left_uid'] = MODULE_PARENT_UID;
			$setting['center_dir'] = DIR_ACTIVE_THEME."/oz.system/user_management/list_user.php";
			$start = 0;
			$itemsPerPage = 15;
			$fields = $objUser->listUserField();
			if(!empty($_GET)){
